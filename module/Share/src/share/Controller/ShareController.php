...
use Ishares\Entity\Answers;
use Ishares\Entity\Questions;
use Ishares\Form\QuestionForm;
...
'questions' => $this->getEntityManager()->getRepository('Ishares\Entity\Questions')->findBy(array(),array('created' => 'DESC')) ));
...
public function addAction()
{
       ...
       // $form->get('submit')->setValue('Add');
	    ...
	    $session->success = 'Data saved successfully';
                ...
                return $this->redirect()->toRoute('ishares');
            }
        }
...
public function editAction()
{
        ...
        if (!$id) {
            return $this->redirect()->toRoute('ishares', array(
                'action' => 'add'
            ));
        }
        $Ishares = $this->getEntityManager()->find('Ishares\Entity\Questions', $id);
        $countries=$this->getEntityManager()->getRepository('SamUser\Entity\Country')->findAll( );
        $ValueOptions=array();
        foreach($countries as $country ){
        $ValueOptions[$country->id]=$country->name;    
        ...
        $form  = new QuestionForm();
        $form->bind($movement);
        $form->get('country')->setValueOptions($ValueOptions)->setValue($Ishares->country) ;
        $form->get('category')->setValue($movement->category) ;
        $form->get('mandatory')->setValue($Ishares->mandatory) ;
...
public function deleteAction(){
     $id = (int) $this->params()->fromRoute('id', 0);
       ...
       $ishares = $this->getEntityManager()->find('Movement\Entity\Questions', $id);    
       $this->getEntityManager()->remove($ishares);
        $this->getEntityManager()->flush();
        $session = new Container('message');
...
->getRepository('Ishares\Entity\Questions')
